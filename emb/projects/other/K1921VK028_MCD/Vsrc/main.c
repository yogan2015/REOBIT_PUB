...
TPark park = PARK_DEFAULTS; //!<Координатные преобразования (программные)
TIPark ipark = IPARK_DEFAULTS; //!<Обратные координатные преобразования (программные)
#elif defined (PARK_IPARK_TMU)
TTMUPark park = TMUPARK_DEFAULTS; //!<Координатные преобразования (аппаратные через блок TMU)
TTMUIPark ipark = TMUIPARK_DEFAULTS; //!<Обратные координатные преобразования (аппаратные через блок TMU)
#endif
...
TDIO dio = DIO_DEFAULTS; //!<Модуль работы с дискретными входами/выходами
...
TTMU tmu = TMU_DEFAULTS; //!<Блок тригонометических преобразований
...
int main(void) {
	...
	SystemCoreClockUpdate();	// Апдейти системных переменных настроенными клоками (чтобы прога знала, какая у неё частота)
...
void TMR1_IRQHandler(void) {
	...
	msCounter++;
	TIsr1 = ((CpuTimerIsr1 - TMR2->VALUE) & 0xFFFFFF) << 1; //время выполнения функции сдвиг на 1 из-за вдвое более медленного тактирования таймера
	if (TIsr1 > 195000) {
...
void TMR0_IRQHandler(void) {
	...
	FastCounter++;
	TIsr10 = ((CpuTimerIsr10 - TMR2->VALUE) & 0xFFFFFF) << 1; //время выполнения функции, сдвиг на 1 из-за вдвое более медленного тактирования таймера
	if (TIsr10 > 19500) {

...
#define ET_AUTO          (v->T_auto < 0)
#define ET_SYNC          (v->T_sync < 0)
...
//  Вне зависимости от режима таймер тикает
  v->T_auto -= v->Td;
  v->T_sync -= v->Td;
...
if ((!CTRL_SYNC_EN)&&(!CTRL_AUTO_EN))  // Начальный режим восстановления
{
  ...
  v->SYNC     = 0;
  v->RESTORE  = 1;
  // оба таймера обновляются
  v->T_auto   = v->Tmin_auto;
  v->T_sync   = v->Tmin_sync;
  goto a;
}
...
if (v->SYNC && ((CTRL_AUTO_EN && COND_AUTO) || EXT_AUTO) && ET_SYNC)  // Переход а авто
{
  v->AUTO     = 1;
  v->SYNC     = 0;
  v->RESTORE  = 0;
// Сброс таймера автоматического режима
  v->T_auto   = v->Tmin_auto;
  goto a;
}

if ((v->AUTO) && CTRL_SYNC_EN && EXT_SYNCREADY && ET_AUTO)  //  Обратная синхронизация из режима разряда
{
  v->AUTO     = 0;
  v->SYNC     = 1;
  v->RESTORE  = 0;
//  Сброс таймера синхронного режима
  v->T_sync   = v->Tmin_sync;
  goto a;
}

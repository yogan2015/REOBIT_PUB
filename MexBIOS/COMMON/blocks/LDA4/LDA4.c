...
#define  ENTER_VKZ_REACH (InArr[21] + DC)
#define ENTER_VKO_REACH (v->Len - InArr[15] - DO)
#define Reach_Cur  InArr[16]
#define EXIT_VKO_ZONE (VKOAREA - 10)
#define EXIT_VKZ_ZONE (VKZAREA + 10)
...
void Exec(void)
    {
      ...
      //(ENTER_VKZ_REACH >= RVMIgnoreArea) ? InArr[21] = RVMIgnoreArea - DC - 1 : InArr[21] = InArr[21];

      if(ENTER_VKZ_REACH >= RVMIgnoreArea)
      {
        InArr[21] = ((RVMIgnoreArea - DC - 1)<=0) ?  0 : (RVMIgnoreArea - DC - 1) ;
      }
      else
      {
        InArr[21] = InArr[21];
...
static void OUTWRITE (BLOCK_STRUCT *v, long *OutArr, long *InArr)
  ...
  static void WORD (BLOCK_STRUCT *v, long *OutArr, long *InArr)
  {
    ...
    if (( v->SW & SW2IE) >>2)  //END POINT SET
    {
      ...
      if (ENTER_VKO_REACH == (v->Len - DO))
      {
        if  (OPCMD &&(*v->CP > (ENTER_VKO_REACH-1) )&& !( v->SW & SW6VKO)&& !v->UPOR)
        {
          v->COMMAND = VKO_INIT;
        }
      }
      else if  (OPCMD &&(*v->CP > (ENTER_VKO_REACH) )&& !( v->SW & SW6VKO)&& !v->UPOR)
      {
        v->Reach = 1;
        LockFlag = 0;
      ...
      if  (*v->CP < (EXIT_VKO_ZONE) ) v->SW &= ~SW6VKO;

      if (ENTER_VKZ_REACH == DC)
      {
        if  (CLCMD &&(*v->CP < (ENTER_VKZ_REACH + 1) )&& !( v->SW & SW7VKZ) && !v->UPOR)
        {
          v->COMMAND = VKZ_INIT;
        }
      }
      else if  (CLCMD &&(*v->CP < ENTER_VKZ_REACH )&& !( v->SW & SW7VKZ) && !v->UPOR)
      {
        v->Reach = 1;
        LockFlag = 0;
        ...
        if  (*v->CP > (EXIT_VKZ_ZONE) ) v->SW &= ~SW7VKZ;
    ...
    if (v->SW & SW2IE)
    {
      ...
      BLOCK CHECK
      ****************************** */
      if ( (inFRVM || ((*v->CI > _IQtoIQ(*v->Fhld,0,22)) && (*v->CS > -v->RVMCS))) && CLCMD && !v->VKO && !v->VKZ && (*v->CP > RVMIgnoreArea)  )//&& (v->MT > _IQ24(1.0))
      {
  ...
  static void PATH (BLOCK_STRUCT *v, long *OutArr, long *InArr)
  {
    ...
    else
    {
      ...
      #define x2 REFARR[i]
      #define x3 *v->CP
//  if ( (PCMD == 0) && (CLCMD || OPCMD)  ||  ( (ALARM > PrevAlarm)||(ALARM < PrevAlarm) )  )
     if ( ( (PCMD == 0) && (CLCMD || OPCMD) ) ||  ( (ALARM > PrevAlarm)||(ALARM < PrevAlarm) ) )
      {

        if ( ((*v->CP > DC+2)&&(*v->CP < v->Len-DO-2)) && ((ALARM > PrevAlarm)||(ALARM < PrevAlarm)) )
        {

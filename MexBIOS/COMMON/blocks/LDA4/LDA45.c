...
static void OUTINIT (BLOCK_STRUCT *v)
{
  v->VKZ = 0; v->VKO = 0; v->RVM= 0; v->RS = 1;  v->SW = 0;  v->DR = 0; v->FR = 50; v->REG = 0; v->OFFSET = 0 ; return;
}
...
static void OUTWRITE (BLOCK_STRUCT *v, long *OutArr, long *InArr)
     ...
     if (!BREAKING)
        {
			 ...
			 if ( !OPCMD && !CLCMD) 
			{ //v->Last_State = State1;
				v->DR = ((( v->SW & SW2IE) >>2)|| v->Reach  ) ? 0 : 1; // no command in init and reverse should set 1 to DR out for stand // || (!v->Reach && (((*v->CP >= ENTER_VKO_REACH) && !OPCMD) || ((*v->CP <= ENTER_VKZ_REACH) && !CLCMD)))
			}
		}
        else
        {
		...
		if( !v->Reach && (((*v->CP >= ENTER_VKO_REACH) && !OPCMD && !CLCMD) || ((*v->CP <= ENTER_VKZ_REACH) && !CLCMD && !OPCMD)) && (( v->SW & SW2IE) >>2) && (!( v->SW & SW7VKZ) || !( v->SW & SW6VKO)))
		{
			v->DR = 0;
		}
  ...
  static void PATH (BLOCK_STRUCT *v, long *OutArr, long *InArr)
  {
    ...
    {//v->LockFlag_Out = State1;
      ...
      ////v->LockFlag_Out = LockFlag;
    }
    else
    {
      if ((v->SW & SW8RVM)&& (v->NS != 1))
      {//v->LockFlag_Out = State2;

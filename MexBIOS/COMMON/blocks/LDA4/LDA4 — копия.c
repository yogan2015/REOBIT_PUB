...
static void MODE     (BLOCK_STRUCT *v)                           ;
static void WORD     (BLOCK_STRUCT *v, long *OutArr, long *InArr);
static void INREAD   (BLOCK_STRUCT *v)                           ;
  ...
  static void INREAD (BLOCK_STRUCT *v)
{
  ...
  if ( (((SB1PT  >= v->PTM/2) && ((SB2PT  < v->PTM/2))) || (inCcmdPT >= v->PTM/2)) != CLCMD ) v->SW ^=SW4CL; // CLOSE to SW
  ...
  ****************************** */
  ...
  if ( PRTIME >= _IQ24(1.5) ) { v->SW = 0; PRTIME = 0;}
...
static void OUTWRITE (BLOCK_STRUCT *v, long *OutArr, long *InArr)
{
  ...
  ****************************** */
  ...
  if ( (v->VKZ && OPCMD) || (v->VKO && CLCMD)) v->SW &= 0xFFFFFF3F;
...
v->FR = (v->MT < _IQ24(0.15) ) ? Fmax: Fmin;
...
if (v->SW & SW6VKO) v->SW &= 0xFFFFFBFF;
  ...
  v->RS = 0; v->RP = 0;  v->SW |= SW2IE; return;
  ...
  if ((*v->CI > _IQ22(0.5))&&(v->REG & 0x4)&&(*v->CP >0) ) v->OFFSET = *v->CP + 1;
  ...
  { v->SW |= SW6VKO;v->RS = 0;  v->RP = 0;  v->SW |= SW2IE; return;}
...
if (!BREAKING) INREAD(v);
  ...
  ****************************** */
  ...
  if ( (*v->CI > _IQ22(0.5)) && CLCMD && !v->VKO && !v->VKZ && (*v->CP > DC+v->OFFSET+FL+SM_C)&& (v->MT > _IQ24(1.0) ) )//(*v->CS > -5) 
  {
...
return;
  ...
  if ((PCMD == 0)&&(CLCMD || OPCMD) )
  {
    ...
    if ((*v->CP <= (DC + v->OFFSET+1) ) || (*v->CP >= (v->Len - DO-1) ))
    {
      ...
      *********************************** */
      ...
      OutArr[0] = DC + v->OFFSET; OutArr[1]  = 30;
      OutArr[16] = v->Len - DO;    OutArr[17] = 30;
      ...
      *********************************** */
      ...
      if (OPCMD)
      {
        ...
        else
        {
          if (*v->CP <= REFARR[12])
          {
            ...
            v->NS = 7;
            OutArr[10] = v->Len - DO;    OutArr[11] = 30;
       ...
       OutArr[12] = v->Len - DO;    OutArr[13] = 30;
       OutArr[14] = v->Len - DO;    OutArr[15] = 30;
       OutArr[16] = v->Len - DO;    OutArr[17] = 30;
          ...
          else
          {
            ...
            OutArr[3] = ((REFARR[15]-(REFARR[15]*SCALE/1000))<30) ? 30 : (REFARR[15]-(REFARR[15]*SCALE/1000));
            OutArr[4] = v->Len - DO;    OutArr[5] = 30;
      ...
      else
      {
       ...
       OutArr[16] = v->Len-DO;        OutArr[17] = 0;
        ...
        OutArr[3] = ( OutArr[2]< REFARR[2] ) ? 30 : Vsyn;
        OutArr[5] = ( (OutArr[4] < REFARR[4] ) && (OutArr[4] >= REFARR[2])  ) ? (Vsyn) : ( (OutArr[4] < REFARR[2] ) ? (30) : (Vmin) );
  ...
  else
  {
    ...
    if ((PCMD != (CLCMD + (OPCMD << 1)) )&& ( _IQtoIQ(*v->CSS,21,0) > 30 ) && !BREAKING  )
    {
     /* **********************************
        BREAK PATH
      *********************************** */
       if ((*v->CP > REFARR[2])&& (*v->CP < REFARR[14])) v->RS = 3; else v->RS = 2;        v->NS = 1;        v->REG |= 0x8;
    v->SW &= ERCMD; v->REG &= 0xFFFFFFFC;
        if (v->DR == 1)
        {
       OutArr[3] =  10;
       OutArr[2] = _IQtoIQ(*v->CPS,20,0)+v->SD+v->SD;
     if (OutArr[2] > v->Len-DO) OutArr[2] = v->Len-DO;
       ...
       OutArr[1] = _IQtoIQ(*v->CSS,21,0)+1;
        ...
        {
       OutArr[1] =  10;
       OutArr[0] = _IQtoIQ(*v->CPS,20,0)+v->SD+v->SD;
